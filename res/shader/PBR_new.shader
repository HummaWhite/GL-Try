...
vec3 addCubeMapUV(vec3 uv, vec2 bias)
{
	vec3 res = uv;
	if (abs(uv.x) > abs(uv.y) && abs(uv.x) > abs(uv.z))
	{
		res += vec3(0.0, bias.x, bias.y);
	}
	if (abs(uv.y) > abs(uv.x) && abs(uv.y) > abs(uv.z))
	{
		res += vec3(bias.x, 0.0, bias.y);
	}
	if (abs(uv.z) > abs(uv.x) && abs(uv.z) > abs(uv.y))
	{
		res += vec3(0.0, bias.x, bias.y);
	}
	return res;
        ...
        float       lightSizeUV)
{
    ...
    {
        float shadowMapDepth = texture(shadowCubeMap, uv + addCubeMapUV(normalize(uv), POISSON_DISK_16[i] * searchWidth * 0.01)).r;
...
float PCFcubeMap(samplerCube shadowCubeMap, vec3 uv, float zReceiver, float diskRad)
{
    ...
    {
        float shadowMapDepth = texture(shadowCubeMap, addCubeMapUV(normalize(uv), POISSON_DISK_16[i] * 0.03)).r;
        shadowMapDepth *= shadowFarPlane;
...
float PCSScubeMap(samplerCube shadowCubeMap, vec3 lightToFrag, float lightSizeUV)
{
    ...
    float filterRad = penumbraSize(zReceiver, avgBlockerDepth) * 0.75;
